...
if __name__ == "__main__":
    lmdb_file = "/vepfs/fs_users/yftc/code/mol2spec_git/data/nega_train_bins1000.lmdb"
    ...
    for p_lr in lr:
        for p_num_layer in num_layer:
            for p_batch_size in batch_size:
                ...
                tailstr = str(p_lr) + "_" + str(p_num_layer) + "_" + str(p_batch_size) + "_bins1000"
                ...
                for epoch in range(num_epochs):
                    ...
                    if (avg_loss < best_loss):  # 仅保存更好的模型
                        best_loss = avg_loss
                        checkpoint_path = os.path.join(Writerpath, f"bins1000_best_model_epoch_{p_lr}_bs{p_batch_size}_nl{p_num_layer}.pt")
                        torch.save({
                            'epoch': epoch + 1,
